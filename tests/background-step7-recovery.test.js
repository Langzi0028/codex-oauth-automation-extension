...
test('step 8 passes LuckMail poll-only long polling options', async () => {
  let capturedOptions = null;
  ...
  assert.equal(capturedOptions.resendIntervalMs, 0);
  assert.equal(capturedOptions.maxAttempts, 20);
...
test('step 8 derives LuckMail poll interval and attempts from configured interval seconds', async () => {
  let capturedOptions = null;
  ...
  assert.equal(capturedOptions.resendIntervalMs, 0);
  assert.equal(capturedOptions.maxAttempts, 9);
