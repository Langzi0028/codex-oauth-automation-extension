...
- `docs/superpowers/plans/2026-05-12-smsbower-provider.md`：SMSBower 接码平台接入实施计划文档，记录 provider adapter、后台配置、手机号验证流、侧栏配置、测试与文档更新任务。
...
- `phone-sms/providers/hero-sms.js`：HeroSMS provider 适配模块，负责基础 URL、国家、价格与余额响应归一化。
- `phone-sms/providers/five-sim.js`：5sim provider 适配模块，负责国家/商品/价格归一化、号码购买、验证码轮询、复用、完成、取消和拉黑操作。
- `phone-sms/providers/registry.js`：手机号接码 provider 注册表，负责 provider ID 归一化、标签和适配模块工厂分发。
- `phone-sms/providers/smsbower.js`：SMSBower provider 适配模块，封装 SMS-Activate 风格 API 的余额查询、号码购买、验证码轮询、状态收尾、国家/服务/价格目录解析，并对外提供统一的 `smsbower` 接码平台契约。
...
- `tests/phone-verification-flow.test.js`：测试手机号验证共享流程对 HeroSMS / 5sim / NexSMS / SMSBower 的取号、复用、轮询、收尾路由，以及重发、换号与 Step 7 重开错误分支。
...
- `tests/sidepanel-phone-verification-settings.test.js`：测试侧边栏接码设置 HTML/JS 接线、HeroSMS / 5sim / NexSMS / SMSBower 配置保存、provider 顺序摘要、手机号注册运行态同步与相关 UI 显隐。
- `tests/smsbower-provider.test.js`：测试 SMSBower provider adapter 的余额、取号、验证码轮询、状态收尾、目录接口解析和 provider registry 接入，所有请求均使用 fake fetch。
